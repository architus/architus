...
impl WAVReceiver {
    ...
    fn close_connection(mut self, connection: &mut TcpStream) {
        let bytes = self.recording_size();
        if bytes == 0 {
            match connection.write(&[ResponseCode::Empty.byte()]) {
                Ok(_) => {}
                Err(_) => warn!("Connection failed for empty recording"),
            }
            return;
        }
        let num_channels = self.audio_channels.len();

        self.balance(true);

        let (url, pw) = match zip(self.audio_channels) {
            Ok(s) => s,
        ...
        msg.push(ResponseCode::Success.byte());
        msg.extend(&(url.len() as u64).to_be_bytes());
        for b in url.as_bytes() {
            msg.push(*b);
        }
        msg.extend(&(pw.len() as u64).to_be_bytes());
        for b in pw.as_bytes() {
            msg.push(*b);
        }
        msg.extend(&(num_channels as u64).to_be_bytes());
        msg.extend(&bytes.to_be_bytes());
        match connection.write(&msg) {
            Ok(_) => info!("Sent file {}", url),
            Err(_) => info!("Failed to send file {}", url),
        }
    }
